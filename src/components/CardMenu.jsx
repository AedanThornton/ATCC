...
const CardMenu = ({ card, flipFunc, secretFunc, setDisplay }) => {
  return (
    <div className="card-menu">
      ...
      {(card.foundIn?.includes("Secret Deck") || card.foundIn?.includes("Envelope") || card.foundIn === "Ultra-secret") && (<button onClick={secretFunc}>{utils.getIcon("Reveal", undefined, undefined, "1.5em")}</button>)}
    </div>
  )
}
